...
# Find the IP address of the current machine                                  current_IP=$(ip addr show | awk '/inet / && !/127.0.0.1/ {print $2}' | cut -d '/' -f 1)
...
if [ ${#target_IPs[@]} -eq 0 ]; then
    echo "No other targets found on the network."
    exit 1                                                                    fi

echo "Target IPs:"  # Display the list of target IPs with a 1-second interval
for target in "${target_IPs[@]}"
do
    echo "$target"  # Display each discovered IP
    sleep 1  # Introduce a 1-second delay between displaying IPs
done
